...
export default function About() {
  ...
  useEffect(() => {
    const observerOptions = {
      root: null,
      threshold: 0.15, // Triggers when 15% of the section is visible in the viewport
    };

    const observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          // Triggers visual reveal state after a 1.5-second (1500ms) delay
          setTimeout(() => {
            entry.target.classList.add('opacity-100', 'translate-y-0');
            entry.target.classList.remove('opacity-0', 'translate-y-8');
          }, 100);

          // Disconnects observer once the animation is queued
  ...
  return (
    <section 
      id="about" 
      ref={sectionRef}
      className="relative z-10 grid grid-cols-1 grid-cols-1 lg:grid-cols-12 gap-12 pt-16 pb-24 items-start transform opacity-0 translate-y-8 transition-all duration-[500ms] ease-out will-change-[opacity,transform]"
    >
